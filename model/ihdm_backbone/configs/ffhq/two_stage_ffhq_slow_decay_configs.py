...
def _ddpm_linear_noise_schedule(boundary_noise_sigma, num_steps, beta_start=1e-4):
    ...
    return np.sqrt(np.maximum(1 - alpha_bar, 0))


def get_config():
    config = default_ffhq_configs.get_default_configs()
    default_model = default_ffhq_configs.get_default_configs().model
    model = config.model
    ...
    model.boundary_noise_sigma = 0.25
    ...
    decay = np.linspace(0.0, 1.0, model.K_blur + 1)
    model.blur_noise_schedule = (
        model.boundary_noise_sigma * (1.0 - decay)
        + model.blur_phase_noise_sigma * decay
    )
    model.blur_noise_schedule[0] = model.boundary_noise_sigma
    model.blur_noise_schedule[-1] = model.blur_phase_noise_sigma

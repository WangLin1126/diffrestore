...
def get_config():
    ...
    model.transition_steps = 8

    model.boundary_noise_sigma = 0.25
    model.sigma = model.boundary_noise_sigma

    model.blur_sigma_min = 0.5
    model.blur_sigma_max = 20
    model.blur_phase_noise_sigma = default_mnist_configs.get_default_configs().model.sigma
    model.blur_phase_signal_scale = 1.0
    model.blur_sampling_noise = 0.0125
